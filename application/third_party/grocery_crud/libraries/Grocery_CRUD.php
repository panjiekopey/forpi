        ...
        foreach($columns as $column)
        {
            if(isset($temp_relation[$column->field_name]))
            {
                ...
                else
                {
                    ...
                    $srch[] = $temp_relation[$column->field_name]; //sama ini
                }
            }
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->order_by				= $this->order_by;
		$data->post_ajax_callbacks = $this->post_ajax_callbacks; //This avails the list of
	...
	protected function get_upload_file_readonly_input($field_info,$value)
	{
		...
		$input .= $is_image ? " $image_class'><img src='".$file_url."' height='50px' sharpen='true'>" : "' target='_blank'>$value";
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	/*
	* edit dari sini http://www.grocerycrud.com/forums/topic/2260-do-javascript-function-after-ajax-list-finished/
 */
    ...
    */
	...
	public function callback_edit_field($field, $callback = null)
	{
		...
		/**
	 *
	 * Used to bypass the dafault formating and allow user to achive custom formatting for the view of a field
	 * @param string $field
	 * @param mixed $callback
	 */
