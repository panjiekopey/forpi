<?php defined('BASEPATH') OR exit('No direct script access allowed');
 /** * For demo purpose only */ class Demo extends MY_Controller
 {
  ...
  {
  // grab records from database table "cover_photos" 		$this->load->model('demo_cover_photo_model', 'photos');
  ...
  { 		$page = $this->input->get('p');
        ...
        // call render() from MY_Pagination 		$this->load->library('pagination');
  ...
  // config from: application/config/pagination.php 		$this->load->library('pagination');
  ...
  {
  // library from: application/libraries/Form_builder.php 		$form = $this->form_builder->create_form();
 		if ($form->validate())
      { 			$this->system_message->set_success('Success!');
 			refresh();
 		}
    // require reCAPTCHA script at page head 		$this->mScripts['head'][] = 'https:
